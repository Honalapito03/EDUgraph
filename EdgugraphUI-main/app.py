...
def upload_document(uploaded_file, title):
    ...
    json.dump(nx.adjacency_data(st.session_state.graphs[title]), open(title + ".json", "w"))
    ...
    if page == "Home":
        ...
        if G is not None:
            ...
            if st.session_state.path == "":
                st.session_state.path = draw_graph(G)   
            ...
            with col1:
                ...
                st.components.v1.html(open(st.session_state.path, 'r').read(), height=600)
        ...
        if st.session_state.graphs:
            ...
            if len(st.session_state.selected_graphlist) == 1:
                    if st.button("Proceed"):
                        ...
                        st.session_state.path = ""
            elif len(st.session_state.selected_graphlist) > 1:
                if st.button("Union"):
